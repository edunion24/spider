...
class Formatter {
  ...
  static String formatName(
    ...
    name = name
        ...
        .replaceFirstMapped(RegExp(r'^[A-Za-z]'), (match) => match.group(0)!.toLowerCase());
    return useUnderScores
        ? name
        : name
            // removes _ and capitalize the next character of the _
            .replaceAllMapped(RegExp(Constants.CAPITALIZE_REGEX), (match) => match.group(2)!.toUpperCase());
  }
  ...
  static String formatFileName(String name) {
    name = name.replaceAllMapped(RegExp(r'[A-Z]+'), (match) => '_${match.group(0)!.toLowerCase()}').replaceFirst(RegExp(r'^_+'), '');
